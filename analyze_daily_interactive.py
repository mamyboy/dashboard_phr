...
function renderMobileCategoryKey(el,items){
  ...
  items.forEach((item,n)=>{const li=document.createElement('li'),badge=document.createElement('b'),name=document.createElement('span');badge.textContent=String(n+1);name.textContent=item.name;li.append(badge,name);list.appendChild(li);});
