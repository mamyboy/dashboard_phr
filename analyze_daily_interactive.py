...
function chartScales(horizontal=false){
  ...
  return horizontal?
    {x:{beginAtZero:true,grace:p.phone?'24%':'16%',grid,ticks:{...ticks,maxTicksLimit:p.phone?5:7}},y:{grid:{display:false},afterFit:axis=>{if(p.phone)axis.width=Math.min(axis.width,118);},ticks:{...ticks,color:'#2e2b31',font:{size:p.phone?8.5:10,weight:'600'},padding:p.phone?4:8}}}:
...
function hbar(el,items){
  const p=viewportProfile(),maxChars=p.tablet?32:52,labels=items.map(i=>p.phone?compactMobileCategory(i.name):wrapChartLabel(i.name,maxChars));
