...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .kpi .v{font-size:34px;font-weight:800;color:#171720;letter-spacing:-.03em} 
  .kpi .l{font-size:13px;color:#3a3a3a;margin-top:4px;line-height:1.4}
...
#dayChips{display:flex;flex-wrap:wrap;gap:6px;align-items:center;max-width:100%;overflow-x:auto;overscroll-behavior-inline:contain;padding:2px 2px 6px;scrollbar-width:none}#dayChips::-webkit-scrollbar{display:none}
  ...
  .kpis{gap:12px}.kpi{position:relative;min-height:112px;padding:13px 14px 12px;border:1px solid rgba(23,23,32,.13);border-radius:18px;box-shadow:0 3px 0 rgba(23,23,32,.08);overflow:hidden}
    ...
    .kpi .v{font-size:34px;font-weight:800;color:#171720;letter-spacing:-.03em}.kpi .l{font-size:13px;color:#3a3a3a;margin-top:4px;line-height:1.4}
...
#unitTable{max-width:100%;width:100%}
  ...
  @media(max-width:680px){
    ...
    .kpis{grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.kpi{min-height:102px;padding:11px;border-radius:15px}.kpi .v{font-size:28px}.kpi .l{font-size:11px}.kpi-top{margin-bottom:7px}
  ...
  @media(max-width:360px){h1{font-size:22px}.kpi .v{font-size:26px}.card{padding-inline:8px}}
...
function hbar(el,items){
  const w=560,row=36,lw=190,pad_t=6;const maxv=Math.max(1,...items.map(i=>i.val));
  ...
  items.forEach((_,i)=>{
    const yy=pad_t+i*row+row/2;
    s+=`<line x1="${lw}" y1="${yy}" x2="${w-18}" y2="${yy}" stroke="${css('--line')}" opacity="0.08"/>`;
  });
  let y=pad_t;
  items.forEach(it=>{const bw=(w-lw-66)*(it.val/maxv);
    ...
    // background track
    s+=`<rect x="${lw}" y="${y+5}" width="${w-lw-66}" height="${row-12}" rx="7" fill="${css('--line')}" opacity="0.2"/>`;
    // value bar
    s+=`<rect class="bar" data-tip="${it.name}: ${it.val}${it.extra||''}" x="${lw}" y="${y+5}" width="${Math.max(bw,3)}" height="${row-12}" rx="7" fill="${it.color}"/>`;
    // value label: place inside bar if enough space, else right-aligned outside
    const labelX = Math.max(bw,3) >= 24 ? lw+Math.max(bw,3)-4 : lw+Math.max(bw,3)+8;
    const labelAlign = Math.max(bw,3) >= 24 ? "end" : "start";
    s+=`<text x="${labelX}" y="${y+row/2}" font-size="11" fill="var(--txt)" dominant-baseline="middle" text-anchor="${labelAlign}" font-weight="600">${it.val}${it.suffix||''} ${it.extra||''}</text>`;
    y+=row;});
  s+='</svg>';el.innerHTML=s;attachTips(el);}
...
function hbar(el,items){
  ...
  const maxLines=Math.max(1,...labels.map(l=>Array.isArray(l)?l.length:1)),row=p.phone?34+maxLines*10:(p.tablet?30+maxLines*9:26+maxLines*8),height=Math.max(p.phone?280:250,Math.min(800,items.length*row+60)));
  ...
  mountChart(el,{type:'bar',data:{labels,datasets:[{label:'ค่า',data:vals,backgroundColor:colors,borderColor:colors,borderWidth:1,borderDash:borderDash,borderSkipped:false,borderRadius:p.phone?8:10,barThickness:p.phone?16:20}]},options:{indexAxis:'y',layout:{padding:{top:8,right:p.phone?50:80,left:0,bottom:4}},scales:chartScales(true),plugins:{datalabels:{display:true,color:'#171720',anchor:'end',align:'right',clamp:true,clip:false,font:{size:p.phone?10:11,weight:'700'},formatter:(v,c)=>`${v}${items[c.dataIndex].extra?' '+items[c.dataIndex].extra:''}`},tooltip:{...chartTooltip(),callbacks:{title:c=>items[c[0].dataIndex].name,label:c=>`${c.raw} ${items[c.dataIndex].tooltipExtra||items[c.dataIndex].extra||''}`}}}}},height);
...
function hbarStacked(el,items,stackKeys,stackColors,stackLabels){
  ...
  // ─── Enhanced colors with better contrast & white borders for separation ───
  const enhancedColors = {
    status_completed: '#059669',      // emerald-600 (darker, more visible)
    status_in_progress: '#1d4ed8',    // blue-700
    status_pending: '#ea580c',        // orange-600
    status_other: '#7c3aed'           // violet-600
  };
  ...
  const datasets=stackKeys.map(key=>({
    label:enhancedLabels[key]||stackLabels[key]||key,
    data:items.map(item=>item.stacks[key]||0),
    backgroundColor:enhancedColors[key]||stackColors[key]||'#999',
  ...
  // ② Add toggle button for Stacked ↔ Grouped view
  const chartKey = `chart-mode-${el.id}`;
  let isStacked = localStorage.getItem(chartKey) !== 'grouped';
  
  function rebuildChart(stacked) {
      const chartOpts = {
        type: 'bar',
        data: { labels, datasets },
        options: {
          indexAxis: 'y',
          layout: { padding: { top: 6, right: p.phone ? 45 : 72, left: 0, bottom: 2 } },
          scales: {
            ...chartScales(true),
            x: { ...chartScales(true).x, stacked, title: { display: true, text: 'จำนวนเคส', font: { size: p.phone ? 10 : 11 } } },
            y: { ...chartScales(true).y, stacked }
          },
          plugins: {
            legend: { display: !p.phone, position: 'bottom', labels: { usePointStyle: true, pointStyle: 'circle', boxWidth: 7, padding: p.phone ? 8 : 11, font: { size: p.phone ? 8 : 9 } } },
            datalabels: {
              display: true,
              color: '#171720',
              anchor: stacked ? 'end' : 'end',
              align: stacked ? 'right' : 'end',
              clamp: true,
              clip: false,
              font: { size: p.phone ? 9 : 10, weight: '700' },
              formatter: (v, c) => {
                const item = items[c.dataIndex];
                const total = Object.values(item.stacks || {}).reduce((a, b) => a + b, 0);
                if (!stacked) return v ? `${v}` : '';
            ...
            tooltip: {
              ...
              callbacks: {
                ...
                label: c => {
                  const item = items[c[0].dataIndex];
                  const val = c.raw;
                  const total = Object.values(item.stacks || {}).reduce((a, b) => a + b, 0);
      ...
      if (btn) btn.textContent = stacked ? '📊 แบบซ้อน (Stacked)' : '📊 แบบแยก (Grouped)';
    }
  
  // Initial render
  rebuildChart(isStacked);
  
  // Add toggle button if not exists
  setTimeout(() => {
    const container = el.parentElement;
    if (!document.getElementById('stackToggle')) {
      const btn = document.createElement('button');
      btn.id = 'stackToggle';
      btn.type = 'button';
      btn.className = 'response-mode';
      btn.style.marginLeft = '8px';
      btn.textContent = isStacked ? '📊 แบบซ้อน (Stacked)' : '📊 แบบแยก (Grouped)';
      btn.onclick = () => {
        isStacked = !isStacked;
        localStorage.setItem(chartKey, isStacked ? 'stacked' : 'grouped');
        rebuildChart(isStacked);
      };
